"""
Multitask multiview mimicgen @84 — semantic action + a UNIFORM aux branch.

The 84-res twin of mimicgen_multitask_sem_plus_uniform_dlp.py (d0 tokens).
Primary (executed) tokenization is semantic: action [3,3,1], proprio [3,6,1].
One auxiliary branch re-tokenizes the SAME noisy action slice per-dimension
(7 tokens, one per scalar):

    [a_pos, a_rot, a_grip, p_pos, p_rot, p_grip, AUX(a_0..a_6), particle_1..K]

Both branches read the same action slice of the same corrupted trajectory, so
the diffusion noise is shared by construction. Both are decoded and take loss
in training; at rollout the aux token still attends but only the primary decode
is executed unless --execute_aux 0 is passed.
...
base = {
    'diffusion': {
        ...
        # Auxiliary tokenization: per-dimension, one token per action scalar.
        'aux_action_token_groups': 'per_dim',
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen84_multitask_sem_plus_uniform/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen84_multitask_sem_plus_uniform/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
