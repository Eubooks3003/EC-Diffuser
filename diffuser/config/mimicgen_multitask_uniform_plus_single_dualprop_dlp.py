"""
Multitask multiview mimicgen — uniform + single, on BOTH action and proprio.

Primary (executed) tokenization is the uniform baseline: one token per scalar
dimension for both action and proprio ('per_dim'). On top of that, one
auxiliary branch re-tokenizes the SAME noisy action slice as a single 7-dim
...
rollout the aux token is still built and still attends, but only the uniform
decode is executed — see policies.py, which reads trajectories[:, :, :A].

Control run: the uniform baseline (mimicgen_multitask_dlp.py, 'per_dim' /
'per_dim'); the auxiliary branch is the only delta.
"""
...
base = {
    'diffusion': {
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen_multitask_uniform_plus_single_dualprop/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen_multitask_uniform_plus_single_dualprop/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
