"""
Multitask multiview mimicgen @84 — SEMANTIC + a SELF-CONDITIONED action token.

Primary (executed) tokenization is semantic action [3,3,1] / proprio [3,6,1]. On top of that sits ONE extra
token that encodes the action as a single token and is NOT noised: it carries
the model's own previous x0 estimate of the action, and the noised action /
proprio / particle tokens self-attend to it.

    [<primary action groups>, <primary proprio groups>, SELFCOND(a_hat0),
     particle_1..K]

This differs from the aux_* arms: those re-tokenize the SAME noised action
slice, whereas this token carries a clean estimate. Un-noised at training means
un-noised at rollout too -- at sampling the token is fed x0_hat from the
previous denoising step (a learned "absent" embedding on the first step, which
is why training drops the token ~50% of the time). No ground truth ever enters
it, so train and eval are identical and rollouts are valid.

The token is input-only: no decoder, no loss. Its job is to be attended to; the
action is already supervised by the primary head.

Control run: mimicgen84_multitask_semantic_dlp.py. The self-cond token is the only delta.
...
base = {
    'diffusion': {
        ...
        # ONE extra un-noised token carrying the model's own previous x0
        # estimate of the action. Input-only; see module docstring.
        'self_cond_action': True,
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen84_multitask_selfcond_plus_semantic/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen84_multitask_selfcond_plus_semantic/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
