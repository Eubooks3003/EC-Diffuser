"""
Multitask multiview mimicgen @84 — uniform action + a single-action AUX branch.

The 84-res twin of mimicgen_multitask_uniform_plus_single_dlp.py (which is on
the old d0 tokens). Primary (executed) tokenization is per-dim uniform: one
token per scalar of action and proprio. On top of that, one auxiliary branch
re-tokenizes the SAME noisy action slice as a single 7-dim token:

    [a_0..a_6, p_0..p_9, AUX(a_all), particle_1..K]

Both branches read the same action slice of the same corrupted trajectory, so
the diffusion noise is shared across representations by construction. Both are
decoded and take loss in training; at rollout the aux token still attends but
only the primary decode is executed (policies.py reads trajectories[:, :, :A]).

Control run: mimicgen84_multitask_uniform_dlp.py. The auxiliary branch is the
only delta.

One token per scalar dimension: 'per_dim' expands to [1]*action_dim and
[1]*gripper_dim at build time, so the value stays dim-agnostic across policies.
The 84 sibling of mimicgen_multitask_dlp.py.
"""
...
base = {
    'diffusion': {
        ...
        # Primary (executed) tokenization: one token per scalar dimension.
        ...
        # primary branch. Weight 1.0 matches the primary action loss scale.
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen84_multitask_uniform_plus_single/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen84_multitask_uniform_plus_single/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
