"""
Multitask multiview mimicgen @84 — semantic action + BOTH single and uniform AUX.

The 84-res twin of mimicgen_multitask_sem_plus_single_uniform_dlp.py (d0
tokens). All THREE tokenizations of the action live in one model. Primary
(executed by default) is semantic: action [3,3,1], proprio [3,6,1]. Two
auxiliary branches re-tokenize the SAME noisy action slice -- once as a single
7-dim token, once per scalar dim:

    [a_pos, a_rot, a_grip, p_pos, p_rot, p_grip,
     AUX0(a_all), AUX1(a_0..a_6), particle_1..K]

All three attend to each other and to the observations. They read the same
action slice of the same corrupted trajectory, so the diffusion noise is shared
by construction. All are decoded and take loss in training; at rollout every
token still attends but only one head is executed --
  (default)         semantic
  --execute_aux 0   single-action
  --execute_aux 1   uniform

Control run: mimicgen84_multitask_semantic_dlp.py.
...
base = {
    'diffusion': {
        ...
        # TWO auxiliary tokenizations of the same action slice: one whole-action
        # token (AUX0) and one token per scalar dim (AUX1). Weight 1.0 puts each
        # aux gradient at the same scale as the primary action loss.
        'aux_action_token_groups': [[7], 'per_dim'],
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen84_multitask_sem_plus_single_uniform/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen84_multitask_sem_plus_single_uniform/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
