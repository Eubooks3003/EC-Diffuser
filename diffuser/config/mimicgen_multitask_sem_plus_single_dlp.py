"""
Multitask multiview mimicgen — semantic action + a single-action AUX branch.
...
noisy action slice as a single 7-dim token. Token sequence:
...
rollout the aux token is still built and still attends, but only the semantic
...
mode_to_args = {
    '12C_dlp': {
        ...
        # Evaluate offline from checkpoints with scripts/eval_paper.py.
        # To watch rollouts locally instead, set eval_freq=100 /
        # eval_backend='mimicgen' (writes eval_videos/step_<N>/<task>/).
        'eval_freq': 0,
        'eval_backend': 'none',
...
base = {
    'diffusion': {
        ...
        # whole 7-dim action, reading the same noisy action slice as the primary
        # branch. Weight 1.0 puts its gradient at the same scale as the primary
        # action loss (drop to 0.5 if the run destabilises vs its baseline).
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen_multitask_sem_plus_single/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen_multitask_sem_plus_single/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
