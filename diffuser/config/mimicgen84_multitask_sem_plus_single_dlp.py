"""
Multitask multiview mimicgen @84 — semantic action + a single-action AUX branch.

The 84-res twin of mimicgen_multitask_sem_plus_single_dlp.py (which is on the
old d0 tokens). Primary (executed) tokenization is the semantic baseline:
action [3,3,1], proprio [3,6,1]. On top of that, one auxiliary branch
re-tokenizes the SAME noisy action slice as a single 7-dim token:

    [a_pos, a_rot, a_grip, p_pos, p_rot, p_grip, AUX(a_all), particle_1..K]

Both branches read the same action slice of the same corrupted trajectory, so
the diffusion noise is shared across the two representations by construction --
no second noise draw, nothing to synchronise. Both are decoded and take loss
during training (the aux projection/decoder must be trained for its token to
mean anything in attention). At rollout the aux token is still built and still
attends, but only the semantic decode is executed -- policies.py reads
trajectories[:, :, :A].

Control run: mimicgen84_multitask_semantic_dlp.py. The auxiliary branch is the
only delta, so the pair isolates what a second tokenization buys.
...
# The 84 store was rendered from the SAME d1/d2 task variants as the 224 store,
# not the _d0 set the original 84-res tokens used (only nut_assembly and
# pick_place stay d0). Tokens from the two variant sets are NOT interchangeable,
# so this must not be pointed at the old d0 pkls. Verified against the store's
# own metadata.json.
TASK_VARIANT = {
    'coffee'                : 'coffee_d2',
...
def _resolve_data_root():
    ...
    candidates = [
        '/lambda/nfs/tal-lpwm-neurips-2026/data/mimicgen_84_wrist_tokens',
        '/home/ellina/Desktop/data/mimicgen_84_wrist_tokens',
    ]
    for c in candidates:
        if os.path.isdir(c):
            return c
...
mode_to_args = {
    '12C_dlp': {
        ...
        'bg_dim': 8,
        'use_bg_obs': True,
        'max_particles': 48,                # covers K=40 (20/view x 2)
        'multiview': True,
        'device': 'cuda:0',
        ...
        # MUST match the views the tokens were encoded from: this store is
        # agentview + robot0_eye_in_hand (wrist), NOT the agentview + sideview
        # the OLD 84-res d0 tokens used. Rendering sideview here would feed wrist
        # token slots with sideview particles -- no crash, just a silent
        # observation-space mismatch that reads as policy failure.
        # Provenance: meta['cameras'] in mimicgen_84_wrist_tokens/*/*.pkl.
        # This store was rendered natively at 84; eval must render at the same
        # size or the DLP sees resized frames it never trained on.
        'mimicgen_preprocess_render_res': 84,
        'mimicgen_cams': ['agentview', 'robot0_eye_in_hand'],
        'mimicgen_camera_width': 84,
        'mimicgen_camera_height': 84,
...
base = {
    'diffusion': {
        ...
        # Primary (executed) tokenization: semantic pos/rot/grip.
        'action_token_groups': [3, 3, 1],
        'proprio_token_groups': [3, 6, 1],

        # Auxiliary tokenization, trained but never executed: ONE token for the
        # whole 7-dim action, reading the same noisy action slice as the
        # primary branch. Weight 1.0 puts its gradient at the same scale as the
        # primary action loss (drop to 0.5 if the run destabilises vs its
        # baseline).
        'aux_action_token_groups': [[7]],
        'aux_action_loss_weight': 1.0,
        ...
        # serialization
        'logbase': logbase,
        'prefix': 'diffusion/mimicgen84_multitask_sem_plus_single/',
    ...
    'plan': {
        ...
        'loadbase': None,
        'logbase': logbase,
        'prefix': 'plans/mimicgen84_multitask_sem_plus_single/',
        'exp_name': watch(args_to_watch),
        'vis_freq': 10,
        'max_render': 8,
