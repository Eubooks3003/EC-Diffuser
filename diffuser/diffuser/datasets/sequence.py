...
class SequenceDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path='', dataset_name='panda_push', horizon=64, obs_only=False,
        ...
        action_normalizer=None, keypose_mode=False, **kwargs):
        ...
        # Auto-correct max_path_length to match the actual loaded pkl. The pkl
        # stores arrays of shape (E, T, ...) and normalize() reshapes via
        # (E * self.max_path_length, -1); if the config disagrees with the pkl's
        # T, the reshape silently produces wrong-shaped tensors. Trust the pkl.
        ...
        # preprocess script (lpwm-copy/scripts/ec_diffuser_rlbench_multiview_preprocess.py)
        ...
        if self.keypose_mode:
            ...
            raw_kp = np.asarray(fields._dict['keypose_indices'], dtype=np.int64)  # (E, max_kp)
            raw_n  = np.asarray(fields._dict['n_keyposes'], dtype=np.int64)       # (E,)
            # Prepend frame 0 to every episode's keypose list (3DDA convention,
            # see 3d_diffuser_actor/data_preprocessing/package_rlbench.py:53).
            # Without this, the model never sees the initial-state obs at training
            # time, but eval starts at frame 0 -> OOD and the policy freezes.
            # Idempotent: skip if frame 0 is already the first keypose (forward-
            # compat with future preprocess runs that do the prepend themselves).
